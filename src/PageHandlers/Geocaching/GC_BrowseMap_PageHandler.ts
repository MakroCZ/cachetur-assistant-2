...
export class GC_BrowseMap_PageHandler extends GC_PageHandlerBase {
    ...
    async initSpecific(): Promise<void> {
        const infoPaneElem = await waitForElement("#map_canvas > div.leaflet-map-pane > div.leaflet-objects-pane > div.leaflet-popup-pane");
    ...
    private mutationsProcessRemove(mutations : MutationRecord[]) {
    ...
    private mutationsProcessAdd(mutations : MutationRecord[]) {
        let addedData = null;
        for (let i = mutations.length - 1; i >= 0; i--) {
